...
@router.get("/all")
def get_all_topics(subject_id: int = None, db: Session = Depends(get_db), admin: User = Depends(get_current_admin)):
    if not subject_id:
...
@router.get("/{topic_id}")
def get_topic(topic_id: int, db: Session = Depends(get_db), admin: User = Depends(get_current_admin)):
...
def create_topic(body: TopicCreate, request: Request, db: Session = Depends(get_db), admin: User = Depends(get_current_admin)):
...
@router.put("/{topic_id}")
def update_topic(topic_id: int, body: TopicUpdate, request: Request, db: Session = Depends(get_db), admin: User = Depends(get_current_admin)):
...
def delete_topic(topic_id: int, request: Request, db: Session = Depends(get_db), admin: User = Depends(get_current_admin)):
...
def add_question(topic_id: int, body: QuestionCreate, request: Request, db: Session = Depends(get_db), admin: User = Depends(get_current_admin)):
...
def add_questions_bulk(topic_id: int, body: BulkQuestionsCreate, request: Request, db: Session = Depends(get_db), admin: User = Depends(get_current_admin)):
...
def update_question(question_id: int, body: QuestionUpdate, request: Request, db: Session = Depends(get_db), admin: User = Depends(get_current_admin)):
...
def download_template(admin: User = Depends(get_current_admin)):
...
async def import_from_docx(topic_id: int, file: UploadFile = File(...), db: Session = Depends(get_db), admin: User = Depends(get_current_admin)):
...
async def upload_question_image(image: UploadFile = File(...), admin: User = Depends(get_current_admin)):
