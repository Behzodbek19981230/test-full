...
@router.get("/all")
def get_all_subjects(db: Session = Depends(get_db), admin: User = Depends(get_current_admin)):
...
def create_subject(body: SubjectCreate, request: Request, db: Session = Depends(get_db), admin: User = Depends(get_current_admin)):
...
def update_subject(subject_id: int, body: SubjectUpdate, request: Request, db: Session = Depends(get_db), admin: User = Depends(get_current_admin)):
...
def delete_subject(subject_id: int, request: Request, db: Session = Depends(get_db), admin: User = Depends(get_current_admin)):
...
async def upload_icon(icon: UploadFile = File(...), admin: User = Depends(get_current_admin)):
