...
def dashboard():
    ...
    for i in range(30):
        ...
        daily_revenue.append({'date': day_start.strftime('%Y-%m-%d'), 'revenue': int(revenue)})
    ...
    for s in subjects:
        topic_count = s.topics.filter_by(is_active=True).count()
        q_count = Question.query.join(Topic).filter(Topic.subject_id == s.id).count()
        attempt_count = TestAttempt.query.filter_by(subject_id=s.id).count()
        subject_stats.append({
            'id': s.id, 'name': s.name, 'icon': s.icon,
            'topic_count': topic_count, 'question_count': q_count,
