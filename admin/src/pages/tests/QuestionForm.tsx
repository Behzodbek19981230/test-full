...
export default function QuestionForm() {
	...
	const handleAiRewrite = async () => {
		if (!form.question_text.trim()) {
			toast.error('Avval savol yozing');
			return;
		}
	...
	const handleCustomPrompt = async () => {
		...
		try {
			const res = await api.post('/ai/assist', {
				...
				text:
					text ||
					"Rasmni tahlil qil va test savoli yarat. Format: Savol: ...\nA) ...\nB) ...\nC) ...\nD) ...\nTo'g'ri javob: A/B/C/D",
				context: '',
	...
	const applyPromptResult = () => {
		...
		if (a && b && c && d) {
			updateForm({
				question_text: qText || form.question_text,
				option_a: a,
				option_b: b,
				option_c: c,
				option_d: d,
	...
	const renderMarkdownLatex = useCallback((text: string) => {
		let html = text
			...
			.replace(/\$\$([\s\S]*?)\$\$/g, (_, tex) => {
				try {
					return `<div style="text-align:center;margin:12px 0">${katex.renderToString(tex.trim(), { throwOnError: false, displayMode: true })}</div>`;
				} catch {
			...
			.replace(
				/^### \*\*(.*?)\*\*$/gm,
				'<h3 style="font-size:15px;font-weight:700;margin:16px 0 8px;color:var(--text-100)">$1</h3>',
			)
			.replace(
				/^### (.*)$/gm,
				'<h3 style="font-size:15px;font-weight:700;margin:16px 0 8px;color:var(--text-100)">$1</h3>',
			...
			.replace(
				/^#### \*\*(.*?)\*\*$/gm,
				'<h4 style="font-size:14px;font-weight:700;margin:12px 0 6px;color:var(--text-200)">$1</h4>',
			)
			.replace(
				/^#### (.*)$/gm,
				'<h4 style="font-size:14px;font-weight:700;margin:12px 0 6px;color:var(--text-200)">$1</h4>',
