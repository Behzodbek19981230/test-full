...
export default function Layout() {
  ...
  useEffect(() => {
    const fetchPending = () => {
      api.get('/payments/pending/count').then(r => setPendingCount(r.data.count)).catch(() => {})
    }
    fetchPending()
  ...
  const navItems: { section: string; items: { to: string; icon: typeof IconLayoutDashboard; label: string; end?: boolean; badge?: number }[] }[] = [
