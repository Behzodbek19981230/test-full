...
export default function Home() {
	...
	const [stats, setStats] = useState<Stats>({ total_users: 0, total_questions: 0, total_attempts: 0, total_subjects: 0 });
					...
					<div className='hero-layout'>
						<div>
							...
							<p className='hero-desc'>
								Abituriyent, o&apos;qituvchi va o&apos;quvchilar uchun maxsus platforma. Sinov
								testlarini ishlang, natijangizni real vaqtda tekshiring.
							</p>
							...
							<div className='hero-trust'>
								...
								<div className='hero-trust-text'>
									<strong>
										{stats.total_users > 0 ? `${stats.total_users.toLocaleString()}+` : '500+'}
									</strong>{' '}
									foydalanuvchi ishonch bildirgan
								</div>
							</div>
						</div>
						...
						{[
							{
								...
								cls: 'purple',
								val: stats.total_users || 500,
								label: 'Foydalanuvchilar',
							},
							{
								icon: <IconClipboardCheck size={24} />,
								cls: 'cyan',
								val: stats.total_questions || 500,
								label: 'Savollar',
							},
							{
								icon: <IconChartBar size={24} />,
								cls: 'green',
								val: stats.total_attempts || 2500,
								label: 'Ishlangan testlar',
							},
							{
								icon: <IconBooks size={24} />,
								cls: 'amber',
								val: stats.total_subjects || 12,
								label: 'Fanlar',
							},
						].map((s, i) => (
				...
				<div className='container'>
					<div className='section-header'>
						<div className='section-label'>
							<IconBooks size={16} /> Fanlar
						</div>
						<h2 className='section-title'>Fanlar va Testlar</h2>
						<p className='section-desc'>
							DTM va attestatsiya uchun barcha fanlar bo&apos;yicha professional testlar
						</p>
					</div>

					<div className='subjects-grid'>
						{subjects.map((s) => (
							<a key={s.id} className='subject-card' href={BOT_URL} target='_blank' rel='noopener noreferrer'>
								<div className='subject-card-icon'>
									<SubjectIcon icon={s.icon} size={52} />
								</div>
								<h3>{s.name}</h3>
								<div className='subject-card-meta'>
									<span>{s.question_count} savol</span>
									<span className='subject-card-price'>{s.price_per_question.toLocaleString()} so&apos;m/test</span>
								</div>
							</a>
						))}
						...
						{subjects.length === 0 && (
							...
							>
								<IconBooks size={48} style={{ opacity: 0.3, marginBottom: '12px' }} />
								<br />
								Fanlar yuklanmoqda...
							</div>
						)}
			...
			<section className='section' id='qanday' style={{ background: 'rgba(255,255,255,0.01)' }}>
				<div className='container'>
					...
					<div className='steps-grid'>
						{[
							{
								num: <IconBooks size={24} />,
								title: 'Fanni Tanlang',
								desc: "Saytda yoki botda kerakli fan va testni ko'ring",
							},
							{
								num: <IconCreditCard size={24} />,
								title: "To'lov Qiling",
								desc: "Karta orqali to'lab chek screenshotini yuboring",
							},
							{
								num: <IconClipboardCheck size={24} />,
								title: 'Testni Ishlang',
								desc: 'Telegram bot orqali qulay interfeys bilan ishlang',
							},
							{
								num: <IconChartBar size={24} />,
								title: "Natijani Ko'ring",
								desc: 'Har bir javobingiz tahlili bilan natijani oling',
							},
						].map((s, i) => (
						...
						{[
							{
								...
								cls: 'purple',
								title: 'Telegram Bot',
								desc: 'Qulay bot orqali istalgan vaqtda, istalgan joyda test ishlang',
							},
							{
								icon: <IconBolt size={26} />,
								cls: 'cyan',
								title: 'Tezkor Tekshirish',
								desc: "Javoblaringiz bir zumda tekshiriladi va batafsil natija ko'rsatiladi",
							},
							{
								icon: <IconChartBar size={26} />,
								cls: 'green',
								title: 'Batafsil Tahlil',
								desc: "Har bir savol bo'yicha to'g'ri va noto'g'ri javoblar tahlili",
							},
							{
								icon: <IconTarget size={26} />,
								cls: 'amber',
								title: 'DTM Formati',
								desc: 'Haqiqiy DTM va attestatsiya formatida professional testlar',
							},
							{
								icon: <IconCreditCard size={26} />,
								cls: 'rose',
								title: "Oson To'lov",
								desc: "Plastik karta orqali qulay va xavfsiz to'lov tizimi",
							},
							{
								icon: <IconBooks size={26} />,
								cls: 'blue',
								title: "Ko'p Fanlar",
								desc: "Barcha DTM fanlari bo'yicha doimiy yangilanib turadigan testlar",
							},
						].map((f, i) => (
					...
					<div className='cta-box'>
						<div className='cta-content'>
							...
							<p className='cta-desc'>
								Minglab abituriyentlar TestFull platformasidan foydalanib, DTMga muvaffaqiyatli
								tayyorlanmoqda
							</p>
