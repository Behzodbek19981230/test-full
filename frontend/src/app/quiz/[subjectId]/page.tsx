        ...
        <div className="quiz-result">
          ...
          <div className="quiz-result__stats">
            <div className="quiz-result__score">
              <div className="quiz-result__score-circle" data-score={result.score >= 70 ? 'good' : result.score >= 50 ? 'mid' : 'low'}>
          ...
          <div className="quiz-result__review">
            ...
            {result.results.map((r, idx) => (
              <div key={r.id} className={`quiz-review-card ${r.is_correct ? 'quiz-review-card--correct' : 'quiz-review-card--wrong'}`}>
                <div className="quiz-review-card__header">
                  <span className="quiz-review-card__num">{idx + 1}</span>
                  <span className={`quiz-review-card__badge ${r.is_correct ? 'quiz-review-card__badge--correct' : 'quiz-review-card__badge--wrong'}`}>
          ...
          <div className="quiz-result__actions">
            <Link href="/#fanlar" className="quiz-btn quiz-btn--ghost">Bosh sahifaga</Link>
            <button className="quiz-btn quiz-btn--primary" onClick={() => window.location.reload()}>Qayta ishlash</button>
          </div>
        </div>
      </div>
    )
    ...
    <div className="quiz-page">
      ...
      <div className="quiz-topbar">
        <div className="quiz-topbar__left">
          <Link href="/#fanlar" className="quiz-topbar__back">← Chiqish</Link>
          <span className="quiz-topbar__subject">{subject?.name}</span>
        </div>
        <div className="quiz-topbar__center">
          <span className="quiz-topbar__progress">{answeredCount}/{questions.length} javob</span>
        </div>
