        ...
        <div className="grid lg:grid-cols-2 gap-20 items-center">
          ...
          <div>
            ...
            <h1 className="text-[58px] font-black leading-[1.08] tracking-[-1.5px] text-slate-50 mb-5 max-lg:text-[40px]">
              ...
              <span className="bg-gradient-to-r from-primary-light to-secondary bg-clip-text text-transparent">Mukammal Tayyorlanish</span>
            </h1>
            ...
            <div className="flex items-center gap-4">
              <div className="flex">
                {['A', 'B', 'S', 'N'].map((l, i) => (
                  <div key={i} className="w-9 h-9 rounded-full border-2 border-dark-900 flex items-center justify-center text-sm font-bold text-white -ml-2 first:ml-0"
                    style={{ background: ['linear-gradient(135deg,#6366f1,#7c3aed)', 'linear-gradient(135deg,#06b6d4,#3b82f6)', 'linear-gradient(135deg,#10b981,#06b6d4)', 'linear-gradient(135deg,#f59e0b,#ef4444)'][i] }}>
                    {l}
                  </div>
                ))}
              </div>
              <p className="text-[13px] text-slate-500">
                <strong className="text-slate-200 font-semibold">500+</strong> foydalanuvchi ishonch bildirgan
              </p>
            </div>
          </div>
          ...
          <div className="hidden lg:flex justify-center" style={{ perspective: 1000 }}>
            <div className="w-full max-w-[400px] bg-dark-800 rounded-3xl border border-white/[0.08] overflow-hidden shadow-2xl [transform:rotateY(-5deg)_rotateX(2deg)] hover:[transform:rotateY(0)_rotateX(0)] transition-transform duration-500">
              ...
              <div className="p-6">
                <div className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2.5">Savol 3</div>
                <p className="text-[15px] font-semibold text-slate-100 mb-4 leading-snug">Agar f(x) = 2x² + 3x - 5 bo&apos;lsa, f(2) ning qiymatini toping.</p>
                <div className="flex flex-col gap-2">
                  {[['A', '7', false], ['B', '9', true], ['C', '11', false], ['D', '13', false]].map(([l, v, c]) => (
                    <div key={l as string} className={`flex items-center gap-3 px-3.5 py-3 rounded-xl border transition-all cursor-pointer
                      ...
                      <span className={`text-sm ${c ? 'text-success font-medium' : 'text-slate-300'}`}>{v as string}</span>
                    </div>
                  ))}
                </div>
              </div>
